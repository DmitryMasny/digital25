 ...
 */
const selectLibraryItem = ({itemId, sourceList, shiftKey, maxSelectCount, lastSelected, selectAll}) => {
    //TODO оптимизировать
    ...
    if (selectAll) {
        return({
            list: itemsArray.map((item)=>{item.selected = true; return item}),
...
const checkStateDifference = (item1, item2, callback) => {
    if (isSameObjects(item1, item2)) {
        ...
        toast.warning('Сохраните или отмените изменения!', {
            autoClose: 3000
        });
        return false;
    }
